...
func TestFailureDomainCheck(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedCauseMessage:  "failed to fetch the failure domain object",
			field:                 field,
		},
		{
			...
			expectedCauseMessage:  "failed to get the prism element cluster",
			field:                 fmt.Sprintf("%s: .spec.prismElementCluster", field),
		},
		{
			...
			expectedAllowed:       false,
			expectedInternalError: true,
			expectedCauseMessage:  "failed to get subnet",
			field:                 fmt.Sprintf("%s: .spec.subnets[%s]", field, subnetName),
		},
		{
