...
func (c *helmAddonConfig) AddFlags(prefix string, flags *pflag.FlagSet) {
	flags.StringVar(
		...
		"default-local-path-csi-helm-values-template",
...
func (s helmAddonStrategy) apply(
	...
	log.Info("Retrieving local-path CSI installation values template for cluster")
	...
	if err != nil {
		return fmt.Errorf(
			"failed to retrieve local-path CSI installation values template for cluster: %w",
			err,
		)
	}
