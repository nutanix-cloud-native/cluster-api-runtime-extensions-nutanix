...
# 4. The resulting JSON array may exceed the Kubernetes ConfigMap size limit of 1 MB. We split it into multiple
#    ConfigMaps, each staying under the limit. The split is done by a helper script that uses a greedy bin-packing
#    algorithm to distribute JSON array elements across chunks.
...
readonly CONFIGMAP_NAME_TEMPLATE="{{ .Values.hooks.cni.calico.crsStrategy.defaultTigeraOperatorConfigMap.name }}"
readonly MAX_CONFIGMAP_BYTES=900000

# Split the JSON array into chunks that each stay under the ConfigMap size limit.
# Each chunk is written to a separate file: tigera-operator-0.json, tigera-operator-1.json, etc.
python3 "${SCRIPT_DIR}/split-json-array.py" \
  --input "${ASSETS_DIR}/tigera-operator.json" \
  --output-dir "${ASSETS_DIR}" \
  --output-prefix "tigera-operator" \
  --max-bytes "${MAX_CONFIGMAP_BYTES}"

readonly OUTPUT_DIR="${GIT_REPO_ROOT}/charts/cluster-api-runtime-extensions-nutanix/templates/cni/calico/manifests"
mkdir -p "${OUTPUT_DIR}"

# Remove any previously generated tigera-operator ConfigMap files.
rm -f "${OUTPUT_DIR}"/tigera-operator-*-configmap.yaml

for chunk_file in "${ASSETS_DIR}"/tigera-operator-*.json; do
  chunk_index="$(basename "${chunk_file}" | sed 's/tigera-operator-\([0-9]*\)\.json/\1/')"
  json_filename="$(basename "${chunk_file}")"

  cat <<EOF >"${OUTPUT_DIR}/tigera-operator-${chunk_index}-configmap.yaml"
...
metadata:
  labels:
    caren.nutanix.com/tigera-operator-manifests: '${CONFIGMAP_NAME_TEMPLATE}'
  name: '${CONFIGMAP_NAME_TEMPLATE}-${chunk_index}'
EOF
done

# Remove the old single-file ConfigMap if it exists (replaced by split files).
rm -f "${OUTPUT_DIR}/tigera-operator-configmap.yaml"
