...
The same suites can run on this machine instead of a DevVM:

```bash
./hack/dev-vm/caren-pc-test.sh run --target local                    # all suites
./hack/dev-vm/caren-pc-test.sh run --target local --suites "unit"    # subset
```

Both targets write the identical `test-results/<ts>/RESULTS.md`. Choose by
suite: platform-neutral suites (precommit, unit) are fine locally; the heavy
kind-based suites and anything needing the Linux toolchain belong on the DevVM.
Known local gap on macOS: nixpkgs-darwin currently ships a golangci-lint built
with an older Go than this repo targets, so `lint` fails locally and passes on
the DevVM — that is the toolchain, not your change.

## Pre-commit hook

```bash
pre-commit run dev-pre-pr --hook-stage manual                    # DevVM (needs NUTANIX_USER/PASSWORD)
TEST_TARGET=local pre-commit run dev-pre-pr --hook-stage manual  # this machine
CAREN_TEST_SUITES="lint unit" pre-commit run dev-pre-pr --hook-stage manual
```

Manual-stage on purpose: the suites take minutes to hours, so they run when the
developer asks, not on every commit.
