...
#   ./hack/dev-vm/caren-pc-test.sh run --target local|devvm --suites "unit lint"   # subset
...
#   4. ships your LOCAL WORKING TREE (uncommitted changes included) and runs the suites
...
# --- local mode ----------------------------------------------------------------
# Same suites, same RESULTS.md, no VM: runs the sibling caren-test-vm.sh suite
# commands directly in this repo. Suits the fast suites (lint, unit) as an
# actual pre-commit check; the heavy kind-based suites are better on a DevVM,
# and a suite that needs what this machine lacks will fail and say so.
run_local() {
  local suites="$1"
  local repo_root
  repo_root=$(git rev-parse --show-toplevel)
  # A GOCACHE shared with a different ambient Go toolchain poisons builds
  # ("compile: version go1.X does not match go tool version go1.Y" - hit live
  # 2026-08-26). Scope the local-mode cache per repo instead.
  GOCACHE="$HOME/.cache/nkp-local-tests/$(basename "$repo_root")/go-build"
  export GOCACHE
  # An exported GOROOT from the developer's shell (~/.zshrc) crosses into
  # devbox's different Go and dies with "compile: version X does not match
  # go tool version Y" - hit live 2026-08-26. Devbox's go knows its own root.
  unset GOROOT
  # Version-sync hooks fetch from api.github.com; unauthenticated they rate-limit
  # at 60/hr, which repeated local runs burn through. Use the ambient gh auth.
  if command -v gh >/dev/null 2>&1; then
    GH_TOKEN="${GH_TOKEN:-$(gh auth token 2>/dev/null)}"
    export GH_TOKEN
  fi
  command -v devbox >/dev/null || die "local mode needs devbox (https://www.jetify.com/devbox)"
  ...
  log "Running suites locally: $suites"
  : >"$results_dir/results.txt"
  local s start rc
  for s in $suites; do
    log "=== $s start ==="
    start=$SECONDS
    if CAREN_REPO_DIR="$repo_root" CAREN_LOCAL_RUN=1 bash "$repo_root/hack/dev-vm/caren-test-vm.sh" "$s" \
        >"$results_dir/suite-$s.log" 2>&1; then
      echo "$s PASS $((SECONDS - start))s" >>"$results_dir/results.txt"
    else
      echo "$s FAIL $((SECONDS - start))s" >>"$results_dir/results.txt"
    fi
    tail -1 "$results_dir/results.txt"
  done

  local branch sha dirty overall
  ...
  grep -q FAIL "$results_dir/results.txt" && overall="FAILED"
  {
    ...
    echo
    echo "- **Branch:** \`$branch\` @ \`$sha\`$([ "$dirty" = true ] && echo " (local working tree, including uncommitted changes)")"
    echo "- **Date:** $(date -u '+%Y-%m-%d %H:%M UTC')"
    echo "- **Runner:** local ($(uname -s) $(uname -m), $(sysctl -n hw.ncpu 2>/dev/null || nproc) cores)"
    echo
    echo "| Suite | Result | Duration |"
    echo "|---|---|---|"
    awk '{printf "| %s | %s | %s |\n", $1, ($2=="PASS"?"✅ PASS":"❌ FAIL"), $3}' "$results_dir/results.txt"
    echo
    echo "_Generated by hack/dev-vm/$(basename "$0") --target local; full logs in this directory._"
  } >"$results_dir/RESULTS.md"
  cat "$results_dir/RESULTS.md"
  log "Results in $results_dir (attach RESULTS.md when you raise the PR)"
    # --pr was already promised in the README; without this it was an unknown flag
    if [ -n "$pr" ]; then
      log "Commenting results on PR #$pr"
      gh pr comment "$pr" --body-file "$results_dir/RESULTS.md"
    fi
...
cmd_run() {
  ...
  local target="${TEST_TARGET:-devvm}"
  local pr=""
  while [ $# -gt 0 ]; do
    case "$1" in
    --suites)
  ...
  local)
    ...
    ;;
  devvm) ;;
  *) die "--target must be 'local' or 'devvm', got '$target'" ;;
  esac
